...
namespace map_internal
{
	...
	{
		DE_ASSERT(n < vector::size(m._data), "Index out of bounds (size = %d, n = %d)", vector::size(m._data), n);
	...
	inline s32 dbg_verify(Map<TKey, TValue>& m, u32 n)
	{
		...
		{
			DE_ASSERT(parent(m, left(m, n)) == n, "Bad RBTree");
			DE_ASSERT(m._data[left(m, n)].pair.first < m._data[n].pair.first, "Bad RBTree");
		}

		if (right(m, n) != m._sentinel)
		{
			DE_ASSERT(parent(m, right(m, n)) == n, "Bad RBTree");
			DE_ASSERT(m._data[n].pair.first < m._data[right(m, n)].pair.first, "Bad RBTree");
		}

		s32 bhL = dbg_verify(m, left(m, n));
		s32 bhR = dbg_verify(m, right(m, n));
		DE_ASSERT(bhL == bhR, "Bad RBTree");
	...
	{
		DE_ASSERT(x < vector::size(m._data), "Index out of bounds (size = %d, n = %d)", vector::size(m._data), x);
...
namespace map
{
	...
	{
		DE_ASSERT(vector::size(m._data) > 0, "Bad Map"); // There should be at least sentinel
